...
# Fonction pour avoir la desc de l'embed selon le nom de la commande qu'on veut afficher la page (pas tres fr mais blc)
...
class Dropdown(discord.ui.Select):
    def __init__(self):
        ...
        options = [
            discord.SelectOption(label=command.name, description=f'Help for /{command.name}') for command in list(bot.tree.get_commands())
        ]

        super().__init__(placeholder='Choose the command ...', min_values=1, max_values=1, options=options)

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.edit_message(embed=discord.Embed(title=f"Help for {self.values[0]}", description=gen_desc(self.values[0])), view=DropdownView())
...
# Help texts
help_texts = {"max": "This command computes the theorical maximum of points (or wins) the player can get in the "
              ...
              "ranking": "This command shows the ranking of the player. The bot simply track the player's ranking every "
                         "15 minutes (if the player is in the top 100), and send a graph with these informations.",
              "points": "This command shows the points of the player. The bot simply track the player's points every 15 "
                        "minutes (if the player is in the top 100), and send a graph with these informations.",
...
# Autocompletion help
async def autocompletion_help(interaction: discord.Interaction, current: str) -> typing.List[
...
class Help(commands.Cog):
    ...
    async def help(self, ctx, cmd: str = None):
        ...
        if cmd is None:
            ...
            await ctx.response.send_message(embed=discord.Embed(title=f"Help for {cmd}", description=gen_desc(cmd)),
                                            view=DropdownView())
